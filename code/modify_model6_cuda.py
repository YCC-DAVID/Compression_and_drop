...
from cad_dataset import CmpDataset,valDataset,CustomCmpBatchDataset,CustomAugmentedDataset,AddGaussianNoise
...
def main():
    ...
    train_loader = DataLoader(
        datasets.CIFAR100(root='/home/chence/workspace/Cmp4ai/comp_and_drop/data', train=True, transform=transforms.Compose([
    ...
    wandb.init(
        ...
        project="Comp And Drop resnet32 if cuda accelerate formal 1",
            ...
            if epoch in args.freez_epoch:
                ...
                if hasattr(args, 'drop') and args.drop:
                    ...
                    for i, batch in enumerate(custom_train_loader):
                        ...
                        # (5B, C, H, W)
                        # aug_ori_feat = aug_output.view(B, N_aug, -1, H, W)
                        # flipped_data = aug_ori_feat[:, 0, :, :, :]    
                        #       # (B, C, H, W)，增强版本0 = flip
                            # (B, C, H, W)
                    ...
                    for j, batch in enumerate(custom_train_loader):
                        ...
                        # aug_tensor = aug_tensor.view(B, N_aug, -1, H, W)

                        # 提取每个增强版本的敏感通道
                        reserved_aug_data = aug_tensor[:,  psnr_index, :, :]  # shape: (B, 5, len, H, W)

                        # reshape 成 (B, 5 * len, H, W)
                        # reserved_aug_data = reserved_aug_data.view(B, -1, H, W)
                    ...
                    if args.cuda_cmp:
                        train_set = PayloadDataset(payloads=cmp_act[0], labels=cmp_act[1])
                        batch_size = 64//args.cmp_batch_size

                        # 2) DataLoader：worker 只做 I/O（不碰 CUDA）
                        train_loader_pre = DataLoader(
                            ...
                            prefetch_factor=2,
                            pin_memory=False,                  # 无 H2D 拷贝，False 即可
                            collate_fn=collate_payload,
                        )

                        # 3) 组装 GPU 侧增强
                        # thr = float(args.drop[drop_flag])      # 你的阈值

                        # gen = torch.Generator(device="cuda").manual_seed(123)
                        thr_now = float(args.drop[drop_flag])

                        def gpu_aug(x, gen=None,thr = thr_now):
                            # 用当前 args/drop_flag，随时能改动
                            
                            # 可选：随机翻转
                            # 两档逻辑：和你原来的顺序一致
                            if thr <= 5:
                                x = gpu_add_gaussian_noise(x, std=0.03, p=1.0, gen=gen)
                                x = gpu_random_crop(x, size=32, padding=4, pad_value=0.0, gen=gen)
                            elif thr <= 10:
                                x = gpu_add_gaussian_noise(x, std=0.01, p=1.0, gen=gen)
                                x = gpu_gaussian_blur(x, kernel_size=(3,3), sigma=(0.1,1.0), gen=gen)
                                x = gpu_random_crop(x, size=16, padding=2, pad_value=0.0, gen=gen)
                            return x # 如需随机翻转设为 0.5
                        gen = torch.Generator(device="cuda").manual_seed(123)     # 可复现实验；不需要可去掉

                        # 4) 包装：主进程 CUDA 解压 + GPU 增强（同一 stream）+ 计时
                        train_loader = ZfpyCudaDecompPrefetch(
                            train_loader_pre, device="cuda", time_unit="ms",
                            tensor_channel=channels, flip_p=0.5,
                            aug_index=select_index,
                            extra_gpu_transform=gpu_aug, gen=gen
                        )

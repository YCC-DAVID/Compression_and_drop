...
from cad_dataset import CmpBatchDataset,valDataset,AddGaussianNoise
import matplotlib.pyplot as plt
...
def zfpy_compress_output(tol):
    def zfpy_cmp_inter(module, input, output):
        # if module.training:
            global inter_tensor,output_inter
            inter_tensor = output
            diff = nn.MSELoss()(output,output_inter)
            trans_str= time.time()
...
def main():
    ...
    #                                                     milestones=[80, 120, 160], last_epoch=0 - 1)
    # lr_scheduler = CubicCosineAnnealingLR(optimizer, args.epoch, eta_min=1e-4, last_epoch=-1, gamma=3.0)
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 160, eta_min=0, last_epoch=-1)
    # gradients = []
    ...
    wandb.init(
        ...
        notes = "verify the performance of batch compression 16 with modified mergemodel method",
    )
    i = 0
    ...
    for epoch in range(0, epoch_num):
                ...
                if hasattr(args, 'drop') and args.drop:
                    if not drop[args.drop[drop_flag]]:
                        if front_model is not None:
                            model = merge_models(front_model,model,resnet32.resnet32(),key_mapping)
                            model = model.to("cuda")
                    ...
                    train_loader_cmp = DataLoader(datasets.CIFAR10(root='./data', train=True, transform=transforms.Compose([
                                                    transforms.ToTensor(),
                                                    normalize,
                                                    ]), download=True), batch_size=16, shuffle=False)
                    ...
                    train_dataset = CmpBatchDataset(cmp_act[0], cmp_act[1], transform=data_augmentation) # 使用 DataLoader 封装数据集
        ...
        if front_model is not None:
            ...
            # mergemodel.eval()
            # model.eval()
            # front_model2,remaining_model2,key_mapping2 = seperate_model(mergemodel,args.drop[frz_flag])
            # front_model2 = front_model2.detach()
            # for i in range(11):
            #     weight_list1 = [[],[]]
            #     weight_list2 = [[],[]]
            #     handle = get_handle_front(mergemodel,args.drop[frz_flag])
            #     if handle is not None:
            #         # hook_handle_f = handle.register_forward_hook(zfpy_compress_output(args.tolerance[frz_flag])) #register compression through hook
            #         # handle_list.append(hook_handle_f)
            #                 # 遍历该 block 内的所有子模块
            #         for name, layer in handle.named_children():
            #             # 检查每个子层是否有 weight 属性
            #             if hasattr(layer, 'weight'):
            #                 if layer.weight is not None:
            #                     weight = layer.weight
            #                     weight_list1[i].append(weight)
            #     for name, layer in front_model[i].named_children():
            #         if hasattr(layer, 'weight'):
            #             if layer.weight is not None:
            #                 weight = layer.weight
            #             weight_list2[i].append(weight)
            # weight_list_diff = [weight_list1[i].data - weight_list2[i].data for i in range(len(weight_list1))]
            # global inter_tensor,output_inter
            # with torch.no_grad():
            ...
            #         input_var = input.cuda()
            #         target = target.cuda()
            #         # output_inter = front_model(input_var)
                    # output_inter2 = front_model2(input_var)
                    # output_m = mergemodel(input_var)

                    # output = model(output_inter)
                    # if handle is not None:
                    #     weight_list1 = []
                    #     weight_list2 = []
                    #     # 遍历该 block 内的所有子模块
                    ...
                    # for name, layer in front_model[11].named_children():
                    #     if hasattr(layer, 'weight'):
                    #         if layer.weight is not None:
                    #             weight = layer.weight
                    #         weight_list2.append(weight)
                    # weight_list_diff2 = [weight_list1[i].data - weight_list2[i].data for i in range(len(weight_list1))]
                    
                    # diff = nn.MSELoss()(output_inter,output_inter2)
                    # acc1 = trainer.accuracy(output, target, topk=(1,))
                    # acc2 = trainer.accuracy(output_m, target, topk=(1,))
                    # acc_diff = acc1[0] - acc2[0]
